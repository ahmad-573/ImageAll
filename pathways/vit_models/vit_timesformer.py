...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x, layer_wise_tokens_norm = self.norm(x), [self.norm(x) for x in layer_wise_tokens]
        return x[:, 0], layer_wise_tokens_norm

    def forward(self, x):
        x, layer_wise_tokens_norm = self.forward_features(x)
        x = self.head(x)
        x_layers = [self.head(x[:,0]) for x in layer_wise_tokens_norm]
        return x_layers
