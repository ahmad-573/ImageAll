...
class resPromptVisionTransformer(Images_VIT):
    ...
    def forward(self, x, all_tokens=True):
        ...
        # x = [self.head_resPrompt(x[:, 0:self.num_prompts].mean(dim=1))]
        return x_resPrompt, [xi[:, self.num_prompts] for xi in layer_wise_tokens_norm]  # , x
